...
#pragma config(Sensor, I2C_1,  clawLeftQuad,   sensorQuadEncoderOnI2CPort,    , AutoAssign )
#pragma config(Sensor, I2C_2,  clawRightQuad,  sensorQuadEncoderOnI2CPort,    , AutoAssign )
#pragma config(Motor,  port1,           liftLeftMid,   tmotorVex393_HBridge, openLoop, reversed)
#pragma config(Motor,  port2,           clawLeft,      tmotorVex393_MC29, openLoop, encoderPort, I2C_1)
...
#pragma config(Motor,  port10,          clawRight,     tmotorVex393_HBridge, openLoop, reversed, encoderPort, I2C_2)
...
void pre_auton()
{
	...
	nMotorEncoder(clawLeftQuad) = 0;
	nMotorEncoder(clawRightQuad)=0;
...
task clawLeftControl(){
	while(true){
		while(runPID_CL){
			currentPosition_CL = nMotorEncoder[clawLeft];
...
task clawRightControl(){
	while(true){
		while(runPID_CR){
			currentPosition_CR = nMotorEncoder[clawRight];
...
task clawInput(){
	while(true){
		if(vexRT[Btn7LXmtr2]==1){
			...
			runPID_CL=false;
			setPosition_CL = nMotorEncoder[clawLeft];
			setClawLeft(127);
		}
		...
		if(vexRT[Btn7LXmtr2]==1){
			...
			runPID_CR=false;
			setPosition_CR = nMotorEncoder[clawRight];
			setClawRight(127);
		}
